...
function resize(){
  ...
  stars=Array.from({length:Math.min(160,Math.floor(innerWidth/8))},()=>({x:Math.random()*innerWidth,y:Math.random()*innerHeight,r:Math.random()*1.2+.2,a:Math.random()*.7+.15,v:Math.random()*.08+.02}));
}
function draw(){
  ctx.clearRect(0,0,innerWidth,innerHeight);
  for(const s of stars){
    s.y+=s.v; if(s.y>innerHeight)s.y=0;
    ctx.beginPath(); ctx.arc(s.x,s.y,s.r,0,Math.PI*2);
    ctx.fillStyle=`rgba(120,205,255,${s.a})`; ctx.fill();
  }
...
const protocolModal=bindModal('protocolModal');
const introModal=bindModal('introModal');
document.querySelectorAll('[data-open]').forEach(b=>b.addEventListener('click',()=>{
  const modal=b.getAttribute('data-open')==='intro'?introModal:protocolModal;
  modal.classList.add('open');
  modal.setAttribute('aria-hidden','false');
}));

const observer=new IntersectionObserver(entries=>entries.forEach(e=>{
  if(e.isIntersecting) e.target.animate([{opacity:0,transform:'translateY(14px)'},{opacity:1,transform:'translateY(0)'}],{duration:520,easing:'cubic-bezier(.2,.7,.2,1)',fill:'both'});
}),{threshold:.08});
